...
const Faq = () => {
  ...
  return (
    <div className='px-4 sm:px-8 md:px-20 lg:px-35 py-10'>
      {/* Section Header */}
      ...
      <div className='flex flex-col w-full'>
        {faqs.map((faq, index) => (
          <div
            key={index}
            className='bg-[#007A5919] w-full md:w-3xl mx-auto mb-5 rounded-md transition-all duration-300'
          >
            <div
              className='flex justify-between px-4 sm:px-5 py-3 items-center cursor-pointer'
              onClick={() => toggle(index)}
            >
              <p className='font-medium text-base sm:text-lg md:text-xl'>{faq.question}</p>
            ...
            {/* Expanded Section */}
            {openIndex === index && (
              <div className='px-4 sm:px-5 pb-3 text-gray-700 text-sm sm:text-base'>
